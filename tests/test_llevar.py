...
import wikt_api as wx, etyobjects, main
...
class TestLlevar:
    ...
    def test_section_detect(self):
        res, dom = fetch_resdom("llevar", redundance=True)
        secs = list(
            eobjects.mwparserhelper.sections_by_level(dom, 3)
        )  # this is catalan
        assert secs == [
            [
    ...
    def test_auto_lang(self, monkeypatch):
        ...
        assert eobjects.mwparserhelper.reduce_to_one_lang(dom) == (
            list(eobjects.mwparserhelper.sections_by_lang(dom, "Catalan")),
            "Catalan",
        )
    ...
    def test_auto_lang_failure(self, monkeypatch):
        ...
        with pytest.raises(MissingException) as e_info:
            eobjects.mwparserhelper.reduce_to_one_lang(dom) == (
                list(eobjects.mwparserhelper.sections_by_lang(dom, "Spanish")),
                "Spanish",
            )
    ...
    def test_connection(self, monkeypatch):
        ...
        Gs = main.mainloop(
            draw_graphs=False,
            test_queries=[("llevaron", "Spanish"), ("llevar", "Spanish")],
        )
        # fetched_Q = fetch_query("llevaron", "Spanish")
        ...
        nx.add_path(
            G_llevar_with_rorigin,
            [
                "$1{m|Latin|levō}",
                "$1{inh|Latin|levāre}",
                "$1{inh|Old Spanish|levar}",
                "$0L{es-verb form of|Spanish|llevar}",
            ],
        )
