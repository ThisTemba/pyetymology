...
import wikt_api as wx, etyobjects, wikt_api
import main
from etyobjects import MissingException
from tests import assets, asset_llevar
import mwparserfromhell as mwp

from tests.assets import asset_llegar
from tests.test_ import (
    fetch_resdom,
    fetch_query,
    fetch_wikitext,
    is_eq__repr,
    patch_multiple_input,
    graph_to_str,
)
...
nx.add_path(
    G_llegar,
    [
        "$0{der|Proto-Indo-European|*pleḱ- ['', 'to plait, to weave']}",
...
class TestLlegar:
    ...
    def test_all_lang_sections(self, monkeypatch):
        ...
        sections = list(
            eobjects.mwparserhelper.all_lang_sections(dom)
    ...
    def test_graph(self, monkeypatch):
        ...
        assert [repr(s) for s in G.nodes] == [
            s for s in reversed(list(G2.nodes))
        ]  # nx reversed the nodes for some reason
        assert [(repr(l), repr(r)) for l, r in G.edges] == [
            e for e in reversed(list(G2.edges))
        ]
    ...
    def test_connection(self, monkeypatch):
        Gs = main.mainloop(
            draw_graphs=False,
            test_queries=[("llegaron", "Spanish"), ("llegar", "Spanish")],
        )
        ...
        nx.add_path(
            G_llegar_with_rorigin,
            [
                "$1{der|Proto-Indo-European|*pleḱ- ['', 'to plait, to weave']}",
    ...
    def test_plico(self):
        ...
        assert (
            graph_to_str(G_plico)
            == "{llegaron#Spanish$0: [], $0L{es-verb form of|Spanish|llegar}: [llegaron#Spanish$0], $1{inh|Latin|plicāre}: [$0L{es-verb form of|Spanish|llegar}], $1{m|Latin|plicō ['I fold']}: [$1{inh|Latin|plicāre}], $1{der|Proto-Indo-European|*pleḱ- ['', 'to plait, to weave']}: [$1{m|Latin|plicō ['I fold']}], $2{der|Proto-Italic|*plekāō}: [$1{m|Latin|plicō ['I fold']}], $2{der|Proto-Indo-European|*pleḱ- ['', 'to plait, to weave']}: [$2{der|Proto-Italic|*plekāō}]}"
        )
