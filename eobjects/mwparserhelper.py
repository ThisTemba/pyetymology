...
def reduce_to_one_lang(
    dom: List[Wikicode], use_lang: str = None, permit_abbrevs=True, use_input=True
) -> Tuple[List[Wikicode], str]:
    """
    ...
    def _compr(found_langs):
        for found_lang in found_langs:
            ...
            h = found_lang[
                2:
            ]  # EDIT: with flat=True, disregard the following. found_lang should be array of length 1, because recursive is false
            yield h[: h.index("==")]

    lang_options = list(_compr(found_langs))
    if len(lang_options) == 0:
        raise MissingException(
            "Zero langs detected !? !?", missing_thing="language_sections"
        )
        ...
        if not lang:
            if use_lang:
                ...
                usrin = fixins.input(
                    "Choose a lang from these options: " + str(lang_options)
                )
            else:  # if such is not possible
                raise MissingException(
                    f"Could not auto-infer language from the languages {str(lang_options)}.",
                    missing_thing="language_specification",
                )
            ...
            if not lang:
                raise MissingException(
                    f'Your input "{usrin}" is not recognized in the options {str(lang_options)}',
                    missing_thing="language_section",
                )
    ...
    if not lang_secs:
        raise MissingException(
            f"No entry was found in wikitext under language {lang} !? !? "
...
def all_lang_sections(
    sections: List[Wikicode], recursive=False, flat=True
) -> Generator[Wikicode, None, None]:
    return sections_by_level(sections, 2, recursive=recursive, flat=flat)


def sections_by_level(
    sections: List[Wikicode], level: int, recursive=True, flat=False
) -> Generator[Wikicode, None, None]:
    """
    ...
    for sec in sections:
        ...
        if has_exact_prefix(sec, prefix):  # we've reached the next header
            if in_section:
                yield yieldme(
                    builder
                )  # if we're already in section, that means yield previous work
            else:
                in_section = (
                    True  # don't yield if we're just starting out, as it will be empty
                )
        ...
        if sec.startswith(
            childprefix
        ):  # if it's a child (this will be skipped until we actually get in to the first section.)
            if recursive:
                builder.append(sec)
            continue
...
def sections_by_lang(
    sections: List[Wikicode], lang: string
) -> Generator[Wikicode, None, None]:
    in_section = False
    for sec in sections:

        if not in_section and sec.startswith(
            "==" + lang
        ):  # we've reached the desired section
            # print(repr(sec))
            in_section = True
            yield sec
            continue
