...
from etyobjects import LemmaRelation, EtyRelation, WordRelation
from emulate import template2url
from emulate.template2url import urllang, urlify
...
class WikiResourceFromQueryStr(WikiResource):
    def __init__(self, word: str, langname: str):
        ...
        self._relation = None
        self.keywordvital = template_word_to_keyword(
            self._relation.word, self._relation.langname
        )
...
class WikiResourceReconstruction(WikiResource):
    def __init__(self, relation: WordRelation):
        ...
        assert relation.word.startswith(
            "*"
        )  # if it's a reconstruction, the template kw should start with a star
    ...
    def _url_vital(
        self,
    ):  # we can put it in _url_vital because this works for both url() and url_api() so I guess wiktionary treats the Reconstruction:<lang> as a discrete unit
