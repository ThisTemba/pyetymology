...
def mainloop(
    ...
    def test_safe_query(original_query, working_G=None):
        ...
        if test_queries:
            if total_queries >= len(test_queries):
                return _EXIT
            _q1 = test_.fetch_query(
                *test_queries[total_queries], query_id=total_queries
            )
        elif run_queries:
            ...
            if potential:
                ...
                _q1 = ety.query(
                    me=potential, query_id=total_queries, working_G=working_G
                )
            else:
            ...
            _q1 = ety.query(
                me=original_query, query_id=total_queries, working_G=working_G
            )
        total_queries += 1
        return _q1
    ...
    while True:
        ...
        G = ety.graph(
            _Q, replacement_origin=GG_origin, cog_search_langs=cog_search_langs
        )
        if test_queries:
            retn.append(G)
        if draw_graphs:
        ...
        if GG_origin:
            ...
            if (
                total_queries != 1
            ):  # warn unconnected queries, unless it's the initial query in which it's OK
